...
# - legislators.csv (legislators and ids from sunlight foundation)
...
$DTA_DIR/bill_metadata.csv: $DTA_DIR/initial_data/extracted_bills_with_sponsers.json
...
$ALIGN_DTA/alignments.csv: data/bill_ids.txt
...
$NCSL_DTA/matched_ncsl_bill_ids.txt: $NCSL_DTA/ncsl_data_from_sample.csv $DTA_DIR/bill_metadata.csv 
	Rscript etl/ncsl/match_bills.R
...
# - re-weigh them by similarity to other alignmnets
# - store all scores without the text
# - Produces a count of alignments in /generate_alignmetns/n_alignments.p
#
## Alignment distribution and alignment descriptives 
$ALIGN_DTA/alignments_notext.csv: $ALIGN_DTA/alignments.csv
	python generate_alignments/process_alignments.py

# Generate the full pairwise alignment set for the ncsl data


# Process that data (re-wiegthing etc.)
