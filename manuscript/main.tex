...
We also rely on an automatic classifier, but we propose a novel approach for
generating training data without relying on human coders. We use the fact that
boiler plate language is highly repetitive within-states. Furthermore, language
that is passed several times within the same state is likely to be not
substantively relevant. Using these facts allows us to accumulate a large amount
of labeled data which we use to train a machine learning classifier to
automatically weed out boiler plate from our alignments. In addition to the text
itself, we use the position in the text as a feature in the classification task.
