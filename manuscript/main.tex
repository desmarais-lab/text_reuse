...
effective indicator of policy similarity. {\bf FRIDO, I DON"T SEE ANYTHING ABOUT THE ADDITIONAL COMPARISONS YOU HAVE DONE HERE. WE DON'T REALLY PROVIDE THE READER WITH A BASELINE FOR COMPARISON HERE. WOULD YOU CALCULATE THE AUCPR OF OUR CLASSIFIER VS A RANDOM CLASSIFIER USING THE PRROC R PACKAGE? ALSO ADD THE VALUE USING THE COSINE SIMILARITY} 
...
& \multicolumn{2}{c}{Identity Link} & \multicolumn{2}{c}{Log Link} \\
  \hline
   & Coefficient & p-value & Coefficient & p-value \\  \hline
...
\hline
\end{tabular}
\caption{Predicting aggregate cross-state alignment scores with diffusion ties. Coefficients calculated with OLS regression. $p$-values based on 5,000 QAP permuations.}
\label{tab:qap.diffusion}
\end{table}

Results of the dyadic regression are presented in Table \ref{tab:qap.diffusion}. In both specifications there is a positive relationship between the number of diffusion ties and the number of alignments. The relationship is statistically significant at the 0.05 level (one tailed) in each analysis. Furthermore, the magnitudes of the relationships are substantively significant. With the identity link, the presence of a diffusion tie leads to a 0.25 standard deviation increase in the expected aggregate alignment score {\bf FRIDO, WOULD YOU VERIFY THE SD NUMBER HERE, OR EVEN JUST STANDARDIZE THE COEFFICIENTS IN THE IDENTITY LINK MODEL BY DIVIDING BY THE SD?}. Based on the log link, the addition of a diffusion tie corresponds to a 30\% increase in the expected aggregated alignment score.\footnote{Calculated as $100\times \left[ \exp(0.266)-1\right] = 30.47$}. These results offer further evidence of the validity of quantifying text re-use as a measure of policy similarity, as the aggregate volume of text re-use between states is positively associated with previously-identified diffusion ties between states.
