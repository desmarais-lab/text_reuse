...
%DIF DEL temp             Tue Dec  8 10:03:29 2015
%DIF ADD text_reuse.tex   Tue Dec  8 10:03:29 2015
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\date{\today}
\author{\DIFaddbegin \DIFadd{Bruce Desmarais, Matt Burgess, Fridolin Linder, Eugenia Giraudi}\DIFaddend }

\maketitle

\begin{abstract} Much research has been focused on the diffusion of policy ideas in US state legislatures. Most of this research uses hand coded data sets that identify equivalent bills and analyze patterns of adoption of these bills. Bills on equivalent policies often contain the same language, since legislators use past legislation from other states or model legislation from interest groups as templates when drafting new bills or model legislation from interest groups. In this paper we evaluate the effectives of text reuse measures to detect bills that address the same policy issues. We find that... 
\end{abstract}
...
\mbox{%DIFAUXCMD
\citet{wilkerson2015tracing} }%DIFAUXCMD
}\DIFaddend use the Smith-Waterman local alignment algorithm \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{smith1981identification}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{(SW algorithm) \mbox{%DIFAUXCMD
\citep{smith1981identification} }%DIFAUXCMD
to detect overlapping language in congressional bills in order to trace policy ideas through the legislative process of the US congress}\DIFaddend . 
...
%DIFDELCMD < \begin{itemize}
%DIFDELCMD < \item %%%
\DIFdel{Policy diffusion (FL expand paragraph)
}%DIFDELCMD < \item %%%
\DIFdel{Text re-use in CBP in bills 
}%DIFDELCMD < \item %%%
\DIFdel{Text re-use in general
}%DIFDELCMD < \begin{itemize}
%DIFDELCMD < \item %%%
\DIFdel{Plaigirism
}%DIFDELCMD < \item %%%
\DIFdel{Alignment (FL expand review of Wilkerson et al)}%DIFDELCMD < \end{itemize} 
%DIFDELCMD < \item %%%
\DIFdel{Discussion of text-based classification
}%DIFDELCMD < \end{itemize}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \section{\DIFadd{Detecting Text Reuse}}
\DIFaddend 

\DIFaddbegin \DIFadd{In this study we use the same algorithm as \mbox{%DIFAUXCMD
\citet{wilkerson2015tracing} }%DIFAUXCMD
to detect matching sequences of text in state bills. We also follow their procedure in splitting the bills up in sections as the unit of analysis. However, our procedure differs in the pre-screening of sections that are compared with the Smith-Waterman algorithm and in the procedure to identify boiler plate text.
}

\DIFadd{\mbox{%DIFAUXCMD
\citet{wilkerson2015tracing}}%DIFAUXCMD
's criterion for pre-screening bills is that they have to match at least 5 10-grams. This means that there has to be a minimum of a 15 character exact match between two bill sections in order to consider them for more in depth analysis with the SW algorithm. We see this as a too restrictive criterion and choose a different approach instead. Our text data is stored in an Elastic Search database. Elastic search is an open source web search engine which is designed to find document that match a search query. We utilize the `more like this' search engine, which is designed to find documents that are similar to a given document }[\DIFadd{FILL IN CITATION}]\DIFadd{. In principle it works in the following way: From the focus document, the $k$ n-grams with the highest tf-idf scores}\footnote{[\DIFadd{Explain these terms in this footnote}]} \DIFadd{are selected and transformed into a search vector. Then, for each document in the collection, a variation of the cosine similarity}\footnote{[\DIFadd{explain cosine similarity, and reference the exact fromula from the documentation}]} \DIFadd{is calculated and the $n$ with the highest scores are selected. This procedure is necessary, since the SW algorithm is computationally demanding and an exhaustive comparison is not feasible (we have about $500,000$ bills, assuming each bill consists on average of four sections this would amount to about $2 * 10^{12}$ comparisons). }[\DIFadd{FILL IN PARAMETER VALUES}]

\DIFadd{Furthermore, we do not rely on human coders to extract and classify boiler plate text. We identified two types of boiler plate text: 1) Procedural language like bill and section headers, common legislative phrases etc. 2) Topic specific boiler plate, like definitions of tax brackets, }[\DIFadd{another example}]\DIFadd{, etc. In order to detect these kinds of boiler plate language, we pre-screen all bill and discard $?-grams$ with a tf-idf score below a certain threshold. Boiler plate text is expected to have very low tf-idf scores, since it appears often across bills but with low frequency within bills. Additonally, in order to detect the second kind of boiler plate, we exclude $?-grams$ that where passes several times in the same legislature, assuming that the exact same provisions will not be introduced several times in the same legislature.
}

\DIFaddend \section{Data}
...
%DIFDELCMD < %%%
\DIFdelend This collection of bills is based on all bills that are available through the \url{openstates.org} API. Open states is a website maintained by the Sunlight Foundation, in order to increase transparency in state politics. The Sunlight Foundation use web scrapers to access all bills that are available on the websites of legislatures in all US states. This includes enacted legislation as well as bill that is still in the legislative process, or where not enacted. The database contains the \DIFdelbegin \DIFdel{full bill text and the following additional metadata:
}\DIFdelend \DIFaddbegin \DIFadd{bill text as well as additional information on the legislative process for the bill. We have the first and the last version of the bill text, the sponsor of the bill and all legislative action that was taken on the bill. 
}\DIFaddend 
...
%DIFDELCMD <     \item %%%
\DIFdel{billdocument first }%DIFDELCMD < \item %%%
\DIFdel{bill document last }%DIFDELCMD < \item %%%
\DIFdel{bill id
    }%DIFDELCMD < \item %%%
\DIFdel{bill title
    }%DIFDELCMD < \item %%%
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \textit{\DIFadd{Ideological Scores}}\\
...
%DIFDELCMD < \begin{itemize}
%DIFDELCMD < \item %%%
\DIFdel{Scrape Google urls for NCSL tables (FL) -- lower priority
    }%DIFDELCMD < \begin{enumerate}
%DIFDELCMD <         \item %%%
\DIFdel{Scraped 64 urls. 
        }%DIFDELCMD < \item %%%
\DIFdel{Can't get around the Google API limitation yet. Also get blocked when trying to scrape the search result. }%DIFDELCMD < \end{enumerate}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \section{\DIFadd{Alignments}}
\DIFaddend 

\DIFdelbegin %DIFDELCMD < \item %%%
\DIFdel{Extract state  \& bill \# from tables (FL) -- lower priority
    }%DIFDELCMD < \begin{enumerate}
%DIFDELCMD <         \item %%%
\DIFdel{Didn't do this yet, let's first check how many are potentially suitable and then probably better to do by hand
    }%DIFDELCMD < \end{enumerate}
%DIFDELCMD < \item %%%
\DIFdel{See tasks in analysis -- create metadata file and dyadic file, then put them on the ACI (FL).
    }%DIFDELCMD < \begin{enumerate}
%DIFDELCMD <         \item %%%
\DIFdel{We have a preliminary dyadic file for all similar bill pairs and their alignment scores (this is from the LID approach: each bill is only compared to }\DIFdelend \DIFaddbegin \DIFadd{The alignment scores are calculated using the affine version of the local alignment algorithm proposed by \mbox{%DIFAUXCMD
...
\citet{wilkerson2015tracing} }%DIFAUXCMD
to trace bills through the legislative process in congress. When considering two bills, }\DIFaddend the \DIFdelbegin \DIFdel{100 most similar ones)
        }%DIFDELCMD < \item %%%
\DIFdel{Matt transfering the database to a new server at the moment, it is therefore unavailable. Should be online again sometime today.
    }%DIFDELCMD < \end{enumerate}
%DIFDELCMD < \item %%%
\DIFdel{Store MALP data on ACI and make sure the identifiers match those in the bill data (FL). }%DIFDELCMD < \begin{enumerate}
%DIFDELCMD <         \item %%%
\DIFdel{data is on aci
        }%DIFDELCMD < \item %%%
\DIFdel{Legislators can be matched by Name, State, Pary and Term. This information should be contained in the database. 
If not, the open state api has a legislator search method, that returns all necessary information.
    }%DIFDELCMD < \end{enumerate}
%DIFDELCMD < \item %%%
\DIFdel{Put policy diffusion data in project folder on ACI (BD).
}%DIFDELCMD < \end{itemize}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{algorithm treats each bill as a sequence of words and tries to find the best alignment between these two sequences. \mbox{%DIFAUXCMD
\citet{wilkerson2015tracing} }%DIFAUXCMD
...
\section{Analysis}
\DIFdelbegin \DIFdel{Below we list X separate analyses designed to test the degree to which measuring text reuse measures policy overlap/diffusion. }\DIFdelend 

\DIFdelbegin %DIFDELCMD < \begin{itemize}
%DIFDELCMD < \item %%%
\DIFdel{We use statistical topic modeling to }\DIFdelend \DIFaddbegin \DIFadd{We use three strategies in order to }\DIFaddend assess the \DIFdelbegin \DIFdel{major content areas represented by the text identified in the alignment algorithm. We consider whether the resultant topics align with policy areas. }%DIFDELCMD < \item %%%
\DIFdel{The Measuring American Legislators Project (MALP) provides data that we will use to assess the significance of text re-use in US state legislation \mbox{%DIFAUXCMD
\citet{shor2011}}%DIFAUXCMD
. The most recent release of the MALP data covers 1993-2014. The MALP data provide ideological scores of legislators on an annual basis.
We conduct a bill-level statistical network analysis to }\DIFdelend \DIFaddbegin \DIFadd{validity of text reuse as a measure of policy equivalence. First, we apply topic models to the sequences that are identified by the SW algorithm as matching sequences. This provides a rough idea of how many of the alignments are substantial and how much procedural language and not substantively significant language gets picked up by the algorithm. Second, we analyze how well the bill similarity aggregated to the state level corresponds with policy diffusion networks identified in previous research \mbox{%DIFAUXCMD
\citep{desmarais2015}}%DIFAUXCMD
. And third, we assess the correlation of the distance of the sponsors of a bill dyad and the dyads alignment scores. If substantive policy content, and importantly, the same ideological direction of the provisions is detected by the alignment algorithm, we will expect a high negative correlation between these two measures.
}


\subsection{\DIFadd{Topic Modeling}}

\DIFadd{We apply statistical topic modeling via Latent Dirichlet Allocation (LDA) \mbox{%DIFAUXCMD
\citep{blei2003} }%DIFAUXCMD
to the alignment text with two objectives.}\footnote{\DIFadd{We use the R package }\texttt{\DIFadd{mallet}} \DIFadd{\mbox{%DIFAUXCMD
\citep{mallet} }%DIFAUXCMD
for the topic modeling application.}} \DIFadd{First, we want to }\DIFaddend see whether the \DIFdelbegin \DIFdel{rate of bill-to-bill alignment is positively related to the ideological similarity of their sponsors. }%DIFDELCMD < \item %%%
\DIFdel{We conduct a state-level analysis }\DIFdelend \DIFaddbegin \DIFadd{major topical themes that emerge are consistent with policy domains that we would expect to be prominently featured in texts that capture the diffusion of public policy. Second, we want }\DIFaddend to see whether \DIFdelbegin \DIFdel{the volume of state-to-state textalignment is positively related to the policy diffusion networks inferred in \mbox{%DIFAUXCMD
\citet{desmarais2015}}%DIFAUXCMD
. }%DIFDELCMD < \end{itemize}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{we can identify topics that correspond to boilerplate/procedural text, which we may want to remove from the alignments to focus the analysis on substantive policy language. In our first cut we use a model with 20 topics. The average alignment is short, with little more than 25\% exceeding 140 characters -- the maximum length of a Tweet. There are 52,735 unique terms in our vocabulary, after removing a standard list of stop words. In Table \ref{tab:top100words} we present the top 100 terms appearing in our corpus. 
}\DIFaddend 

\DIFaddbegin \begin{table}[ht]
\DIFaddFL{\rule{\textwidth}{1pt} }\\ \vspace{-1cm}
\begin{quotation}
\noindent \DIFaddFL{state person section states act means health public member general commission department law information provided insurance including board subsection legislature benefit court individual property order services care action business time pursuant united date year subject interstate federal compact school insurer corporation required medical chapter provide party entity vehicle amount notice people assembly agreement tax qualified provisions attorney agency limited made service authority child purposes years commissioner education report days policy employer members company code contract employee interest authorized effective statement district purpose ii application period number credit plan include senate income system amended issued reasonable part program director patient requirements
}\end{quotation} \vspace{-.5cm}
\DIFaddFL{\rule{\textwidth}{1pt} }\vspace{-.2cm}
\caption{\DIFaddFL{Top 100 words, listed left-to-right, in the alignments corpus.}}
\label{tab:top100words}
\end{table}

\DIFadd{The topic modeling results are presented in Table \ref{tab:topics}. We see that most topics correspond to recognizable policy domains. However, some topics, such as Topic 9, which appears to be a mix of automobile policy and abortion policy, appear to consist of more than one distinct policy domain. As such, we should try running this with more topics. Topic 17 appears to be a boilerplate topic. 
}
...
\begin{tabular}{rl}
  \hline
  \hline
\DIFaddFL{6 }& \DIFaddFL{benefit corporation public director officer specific interests directors person purpose }\\ 
  \DIFaddFL{16 }& \DIFaddFL{state order child section support law application employer enforcement court }\\ 
  \DIFaddFL{14 }& \DIFaddFL{salts isomers food substances substance means preparation product compound mixture }\\ 
  \DIFaddFL{3 }& \DIFaddFL{court state order person petition section proceeding law appraisal jurisdiction }\\ 
  \DIFaddFL{8 }& \DIFaddFL{feet distance medical patient marijuana point qualifying south north west }\\ 
  \DIFaddFL{20 }& \DIFaddFL{information electronic consumer service section insurance notice portable electronics means }\\ 
  \DIFaddFL{9 }& \DIFaddFL{vehicle motor person means abortion woman medical child dealer death }\\ 
  \DIFaddFL{18 }& \DIFaddFL{property interest statement section person trust real transfer agreement financing }\\ 
  \DIFaddFL{5 }& \DIFaddFL{person offense age sexual years criminal court officer sex guilty }\\ 
  \DIFaddFL{15 }& \DIFaddFL{states united state presidential election official member vote president popular }\\ 
  \DIFaddFL{17 }& \DIFaddFL{state enacted legislature general assembly people amended section read senate }\\ 
  \DIFaddFL{2 }& \DIFaddFL{action person section court state civil violation attorney reasonable degree }\\ 
  \DIFaddFL{7 }& \DIFaddFL{qualified tax section investment revenue entity credit amount fund code }\\ 
  \DIFaddFL{19 }& \DIFaddFL{health care patient practice medical physician licensed person means services }\\ 
  \DIFaddFL{12 }& \DIFaddFL{department health state services act public year federal care program }\\ 
  \DIFaddFL{4 }& \DIFaddFL{appointed members house attorney senate power principal member authority representatives }\\ 
  \DIFaddFL{1 }& \DIFaddFL{individual employer year years taxable service income employee period january }\\ 
  \DIFaddFL{10 }& \DIFaddFL{insurance insurer policy commissioner company section coverage contract state plan }\\ 
  \DIFaddFL{11 }& \DIFaddFL{state commission interstate compact member states rules compacting effective law }\\ 
  \DIFaddFL{13 }& \DIFaddFL{school student board district education public charter state students county }\\ 
   \hline
\end{tabular}
\caption{\DIFaddFL{Results from 200 iterations of LDA with 10 iterations of hyper parameter optimization. Top 10 words in each topic listed for each topic. Topics listed in order of frequency of appearance within documents, beginning in the first row of the table with the most frequent topic.}}
\label{tab:topics}
\end{table}
...
\subsection{\DIFdelbegin \DIFdel{Tasks}\DIFdelend \DIFaddbegin \DIFadd{Ideological Distance of Aligned Bills}\DIFaddend }
\DIFdelbegin %DIFDELCMD < \begin{itemize}
%DIFDELCMD < \item %%%
\DIFdel{Develop bill metadata dataset }\DIFdelend \DIFaddbegin 

%DIF > R script for this analysis: '/analysis/ideology.R'

\DIFadd{The MALP dataset contains ideal points for about $20,000$ state legislators. The }\texttt{\DIFadd{openstates}} \DIFadd{API contains data and identifiers on $12,000$ legislators. Of these we where able to uniquely match about $8,000$ legislators. This allowed us to obtain ideal points for 395,474 ($69\%$) bills and 1,745,336 }\DIFaddend (\DIFdelbegin \DIFdel{FL) }%DIFDELCMD < \begin{itemize}
%DIFDELCMD < \item %%%
\DIFdel{unique bill identifier
}%DIFDELCMD < \item %%%
\DIFdel{sponsor identifier from  MALP data
}%DIFDELCMD < \item %%%
\DIFdel{state identifier
}%DIFDELCMD < \item %%%
\DIFdel{chamber identifier
}%DIFDELCMD < \item %%%
\DIFdel{Introduction date
}%DIFDELCMD < \item %%%
\DIFdel{Anything else, even if some is missing (status, committees, etc) }%DIFDELCMD < \end{itemize}
%DIFDELCMD < \item %%%
\DIFdel{Develop bill-to-bill edgelists (FL) }%DIFDELCMD < \begin{itemize}
%DIFDELCMD < \item %%%
\DIFdel{Sparse dyadic dataset (i.
e. , no observations when there is 0 alignment/overlap)}%DIFDELCMD < \item %%%
\DIFdel{alignment score (s) }%DIFDELCMD < \item %%%
\DIFdel{name of a text file in which aligned text is stored
}%DIFDELCMD < \end{itemize}
%DIFDELCMD < \item %%%
\DIFdel{Topic models, possibly of a relational form, applied to aligned text }%DIFDELCMD < \item %%%
\DIFdel{Computationally intensive analysis of bill-to-bill alignment and sponsor ideology.
}%DIFDELCMD < \begin{itemize}
%DIFDELCMD < \item %%%
\DIFdel{Get big sample of bills
}%DIFDELCMD < \item %%%
\DIFdel{Assess relationship between alignment between bills }\DIFdelend \DIFaddbegin \DIFadd{$50\%$) pairs of bills. 
}

\DIFadd{In order to assess the validity of text reuse as a measure of substantive policy overlap, we expect the quality of the alignments to be inversely correlated to the distance of the bills sponsors' ideal points. The MALP ideal points are located on a common scale across all states, the distance between sponsors from different states is therefore meaningful. 
}

\DIFadd{In the following sections we present several analyses to assess this correlation. When we are generating the alignments the bills are split up into sections. Table \ref{tab:ideo_reg} displays results for several methods of combining multiple alignments across sections of the same bill dyad as well as treating sections as the unit of analysis. For the case where we aggregate to the bill level, simple bivariate linear regression is used to test for a correlation, since the data has network structure we calculate standard errors with the quadratic assignment procedure (QAP) }[\DIFadd{note, Table \ref{tab:ideo_reg} still has normal standard errors, we are runnign the QAP at the moment}] \DIFadd{\mbox{%DIFAUXCMD
\citep{krackardt1987qap}}%DIFAUXCMD
. There are six models in Table \ref{tab:ideo_reg}. The unit of analysis of the first model is the bill section, the other five models are on the bill level. Model (1) has the alignment score of the section dyad as the dependent variable, models (2) - (5) use different methods to aggregate the scores on the section - dyad level to the bill - dyad level.
}

\input{tables/ideology_regressions.tex}
