...
class LID(object):
    ...
    def get_bill_list_alignments(self, left_doc, right_docs, **kwargs):

        '''
        Generate alignments of focus bill with specified list of 'right bills'

        left_doc: document to compare to list
        right_doc: List of bill ids to calculate alignments for
        ...
        # Retrieve list of right bills from elastic search
        
        # Function not written yet
        result_docs = self.elastic_connection.retrieve_bills(bill_ids,
        ...
        for i,result_doc in enumerate(result_docs):
            ...
            # This part excludes same-state results and results below a
            # lucene threshold (commented out for text_reuse paper):
            ...
            #    break
            if result_doc['bill_document_last'] is None:
                continue
    ...
    def find_state_bill_alignments(self, query_document,document_type="text",
            split_sections=False, **kwargs):
        '''
        ...
        know section chunking will be done on the query, other options include 
        state bill tuples i.e ("state_bill","al") and "model_legislation"

        split_sections: specifies whether the query document will be broken into 
        sections to find multiple alignments (True) or whether to treat the 
        documents as one and identify a single best alignment (False)                            
        '''
        ...
        if document_type == "state_bill":
            try:
                ...
                raise LidException("if document type is state_bill then you musy specify state_id and query_document_id")
        ...
        for i,result_doc in enumerate(result_docs):
            ...
            if result_doc['id'] == kwargs['query_document_id']:
                continue

            if result_doc['bill_document_last'] is None and \
               result_doc['bill_document_first'] is None:
                continue
            ...
            try:
                ...
                result_sequence = clean_document(result_doc['bill_document_first'],
                        state_id = result_doc['state'])[0]
    ...
    def find_evaluation_alignments(self, query_document, document_type="text",
            split_sections=False, **kwargs):
        '''
        query_document: query document, usually in the form of an entire bill, 
        model legistlation or segment of either
